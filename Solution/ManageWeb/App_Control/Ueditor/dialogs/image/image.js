...
(function () {
    ...
    function insertBatch() {
        ...
        for (var i = 0, ci; ci = imageUrls[i++];) {
            ...
            if (tmpObj._src.indexOf("w=") > 0 && tmpObj._src.indexOf("?") > 0) {
              
                try {
                    var width = getQueryValue(tmpObj._src,"w");
                    var height = getQueryValue(tmpObj._src,"h");
                    // 判断是否有缓存
                    if (width != null && height != null) {
                        tmpObj.width = width;
                        tmpObj.height = height;
                        tmpObj.style = "width:" + width + "px;height:" + height + "px;";
                    }
    ...
    function getQueryValue(urlValue,key) {
        var queryParam = urlValue.split("?")[1];
    ...
    function createAlignButton(ids) {
        for (var i = 0, ci; ci = ids[i++];) {
            var floatContainer = g(ci),
                nameMaps = { "center": lang.floatCenter, "none": lang.floatDefault, "left": lang.floatLeft, "right": lang.floatRight };

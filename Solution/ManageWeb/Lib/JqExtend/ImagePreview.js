...
function OpenImagePreview(e) {
    if (e.src.indexOf("data:image") >= 0) {
        ...
        window.open(e.src, 'newwindow', 'left=150,top=120');
    }
}

$('script[src*=ImagePreview]').each(function () {
    var s = this.src;
    if (s.match(/ImagePreview[^\/]*\.js/i)) { PreviewRoot = s.replace(/[\?#].*$/, '').replace(/(^|[\/\\])[^\/]*$/, '$1'); return false; }
});
var ImagePreview = {};
//透明图片
...
ImagePreview.MaxWidth = 0;
ImagePreview.MaxHeight = 0;

//chrome浏览器远程预览地址
ImagePreview.Action = null;

ImagePreview.Preview = function (file, imgexp) {
    ...
    ImagePreview.MaxWidth = img.width();
    ImagePreview.MaxHeight = img.height();
            ...
            var _upload = new QuickUpload(file, {
                ...
                onFinish: function (iframe) {
                    try {
                        ...
                        if (iframe.contentWindow.document.body.innerHTML.indexOf("data:image") >= 0) {
...
QuickUpload.prototype = {
    ...
    _setIframe: function () {
        if (!this._iframe) {
            ...
            var iframeElement = "<iframe name=\"" + iframename + "\">";
            if ($.browser.msie) {
            ...
            iframe = document.createElement(iframeElement);
    ...
    _setForm: function () {
        ...
        this._form.action = PreviewRoot + "ImgPreview.ashx?" + jQuery.param(this.parameter);
    },
    //设置input
    _setInput: function () {

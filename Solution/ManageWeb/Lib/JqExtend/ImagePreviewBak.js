...
var ImagePreview = {};
//透明图片
ImagePreview.TRANSPARENT = "data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==";

//文件上传控件
ImagePreview._File = null;

//图片预览对象
ImagePreview._Image = null;

// 该对象只用来在IE下获得图片的原始尺寸,无其它用途
ImagePreview._PreviewSizeFake = null;

ImagePreview.MaxWidth = 240;
ImagePreview.MaxHeight = 320;

//chrome浏览器远程预览地址
ImagePreview.Action = null;

ImagePreview.Preview = function (file, img) {

    ImagePreview._File = file;

    ImagePreview._Image = img;
    $(img).attr("src", ImagePreview.TRANSPARENT);
    $(img).show();

    if (ImagePreview._PreviewSizeFake == null) {
        if ($("#preview_size_fake").length > 0)
            ImagePreview._PreviewSizeFake = $("#preview_size_fake");
        else {
            $("body").append("<img id='preview_size_fake' />");
            ImagePreview._PreviewSizeFake = $("#preview_size_fake");
        }
    }
    ...
    if ($.browser.msie) {
        if ($.browser.version == 6.0) {
            ...
            $(ImagePreview._PreviewSizeFake).attr("src", src);
            $(img).attr("src", src);
            $(img).css("width", 240);
            setTimeout("ImagePreview.PreviewTimeout();", 100);
        }
        else {
            if (ImagePreview._PreviewSizeFake == null) {
                if ($("#preview_size_fake").length > 0)
            ...
            if (imgSrc != "" && !imgSrc.match(/.jpg|.gif|.png|.bmp/i)) {
                alert('图片格式无效！');
                return false;
            }

            //添加滤镜
            $(ImagePreview._Image).attr("src", ImagePreview.TRANSPARENT);
            $(ImagePreview._Image).css("filter", "");
            $(ImagePreview._Image)[0].style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod='scale')";
            $(ImagePreview._Image)[0].filters.item('DXImageTransform.Microsoft.AlphaImageLoader').src = imgSrc;

            //真实宽度对象
            $(objPreviewSizeFake).css("visibility", "hidden");
            $(objPreviewSizeFake)[0].style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod='image')";
            $(objPreviewSizeFake)[0].filters.item('DXImageTransform.Microsoft.AlphaImageLoader').src = imgSrc;
            setTimeout("ImagePreview.PreviewTimeout();", 100);
        }
    }
    if ($.browser.mozilla) {
        var url = file.files[0].getAsDataURL();
        $(ImagePreview._PreviewSizeFake).attr("src", url);
        $(img).attr("src", url);
        $(img).css("width", 240);
        setTimeout("ImagePreview.PreviewTimeout();", 100);
    }
    if ($.browser.opera || $.browser.safari) {
        ...
        var _upload = new QuickUpload(file, {
            onReady: function () {
                this.action = ImagePreview.Action;
                var parameter = {};
                //parameter.ratio = oThis.ratio;
                parameter.width = ImagePreview.MaxWidth;
                parameter.height = ImagePreview.MaxHeight;
            },
            onFinish: function (iframe) {
                try {
                    $(img).attr("src", iframe.contentWindow.document.body.innerHTML);
                    _upload.dispose();
                } catch (e) {
                }
            },
...
//IE、FF设置图片大小函数
ImagePreview.PreviewTimeout = function () {
    //缩放
    var offsetWidth = $(ImagePreview._PreviewSizeFake)[0].offsetWidth;
    var offsetHeight = $(ImagePreview._PreviewSizeFake)[0].offsetHeight;

    //长宽比例
    var desiredRatio = Math.min(ImagePreview.MaxWidth / offsetWidth, offsetHeight / ImagePreview.MaxHeight);

    var width = offsetWidth * desiredRatio;
    var height = offsetHeight * desiredRatio;
    $(ImagePreview._Image).css("width", width);
    $(ImagePreview._Image).css("height", height);
};

namespace WTF.Framework
{
    ...
    public class BlowFishCrypToHelper
    {
        private uint[] bf_P;
        private uint[] bf_s0;
        private uint[] bf_s1;
        ...
        public BlowFishCrypToHelper(string hexKey)
        {
            if ((hexKey.Length % 8) != 0)
            {
        ...
        public BlowFishCrypToHelper(byte[] cipherKey)
        {
            if ((cipherKey.Length % 8) != 0)
            {
                throw new Exception("key的长度必须为8的倍数,并且不能大于56位");
            }
            this.randomSource = new RNGCryptoServiceProvider();

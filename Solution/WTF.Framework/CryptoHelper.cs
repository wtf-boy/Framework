namespace WTF.Framework
{
    ...
    public class CryptoHelper
    {
        ...
        public CryptoHelper(string blowfishKey)
        {
            if ((blowfishKey.Length % 8) != 0)
            {
                throw new Exception("key的长度必须为8的倍数,并且不能大于56位");
            }
            if (this.randomSource == null)
            {
                this.randomSource = new RNGCryptoServiceProvider();
            }
            this.SetupKey(this.HexToByte(blowfishKey));
        }

        public static byte[] AESDecrypt(byte[] toArray, string key)
        {
        ...
        public static byte[] AESEncrypt(string toArray, string key)
        {
            byte[] bytes = Encoding.UTF8.GetBytes(key);
        ...
        private byte[] Crypt_CBC(byte[] text, bool decrypt)
        {
            ...
            Buffer.BlockCopy(text, 0, dst, 0, text.Length);
            byte[] buffer2 = new byte[8];
            byte[] buffer3 = new byte[8];
            byte[] buffer4 = new byte[8];
            Buffer.BlockCopy(this.InitVector, 0, buffer4, 0, 8);
            if (!decrypt)
            {
                for (num2 = 0; num2 < dst.Length; num2 += 8)
                {
                    Buffer.BlockCopy(dst, num2, buffer2, 0, 8);
                    this.XorBlock(ref buffer2, buffer4);
                    this.BlockEncrypt(ref buffer2);
                    Buffer.BlockCopy(buffer2, 0, buffer4, 0, 8);
                    Buffer.BlockCopy(buffer2, 0, dst, num2, 8);
                }
                return dst;
            }
            for (num2 = 0; num2 < dst.Length; num2 += 8)
            {
                Buffer.BlockCopy(dst, num2, buffer2, 0, 8);
                Buffer.BlockCopy(buffer2, 0, buffer3, 0, 8);
                this.BlockDecrypt(ref buffer2);
                this.XorBlock(ref buffer2, buffer4);
        ...
        private byte[] Crypt_ECB(byte[] text, bool decrypt)
        {
            ...
            {
                Buffer.BlockCopy(dst, i, buffer2, 0, 8);
                if (decrypt)
                {
                    this.BlockDecrypt(ref buffer2);
                }
                ...
                Buffer.BlockCopy(buffer2, 0, dst, i, 8);
            }
            return dst;
        }
        ...
        {
            this.xl_par ^= this.bf_P[0x11];
            for (uint i = 0x10; i > 0; i -= 2)
            {
                this.xr_par = this.round(this.xr_par, this.xl_par, i);
                this.xl_par = this.round(this.xl_par, this.xr_par, i - 1);
            }
        ...
        {
            this.IV = this.HexToByte(ct.Substring(0, 0x10));
            return Encoding.ASCII.GetString(this.Decrypt_CBC(this.HexToByte(ct.Substring(0x10)))).Replace("\0", "");
        }

        private byte[] Decrypt_CBC(byte[] ct)
        {
        ...
        {
            return Encoding.ASCII.GetString(this.Decrypt_ECB(this.HexToByteDecyp(ct))).Replace(" ", "");
        }
        ...
        private byte[] Encrypt_CBC(byte[] pt)
        {
        ...
        {
            int totalWidth = ((pt.Length % 8) == 0) ? pt.Length : ((pt.Length + 8) - (pt.Length % 8));
            if ((pt.Length % 8) != 0)
            {
                pt = pt.PadRight(totalWidth, ' ');
            }
            return this.ByteToHex(this.Encrypt_ECB(Encoding.ASCII.GetBytes(pt)));
        }

        private byte[] Encrypt_ECB(byte[] pt)
        {
        ...
        private void GetBlock(ref byte[] block)
        {
            ...
            if (this.nonStandardMethod)
            {
                ...
                bytes = BitConverter.GetBytes(this.xl_par);
            }
            else
            {
                array = BitConverter.GetBytes(this.xl_par);
                bytes = BitConverter.GetBytes(this.xr_par);
                Array.Reverse(array);
                Array.Reverse(bytes);
            }
            Buffer.BlockCopy(array, 0, block, 0, 4);
            Buffer.BlockCopy(bytes, 0, block, 4, 4);
        }
        ...
        private byte[] HexToByteDecyp(string hex)
        {
        ...
        {
            uint num = (this.bf_s0[this.wordByte0(b)] + this.bf_s1[this.wordByte1(b)]) ^ this.bf_s2[this.wordByte2(b)];
            uint num2 = num + this.bf_s3[this.wordByte3(b)];
            uint num3 = num2 ^ this.bf_P[n];
            return (num3 ^ a);
        }
        ...
        private void SetBlock(byte[] block)
        {
            ...
            Buffer.BlockCopy(block, 0, dst, 0, 4);
            Buffer.BlockCopy(block, 4, buffer2, 0, 4);
            if (this.nonStandardMethod)
            {
                this.xr_par = BitConverter.ToUInt32(dst, 0);
                this.xl_par = BitConverter.ToUInt32(buffer2, 0);
            }
            else
            {
                Array.Reverse(dst);
                Array.Reverse(buffer2);
                this.xl_par = BitConverter.ToUInt32(dst, 0);
                this.xr_par = BitConverter.ToUInt32(buffer2, 0);
            }
        }
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            if (cipherKey.Length > 0x38)
            {
                throw new Exception("Key too long. 56 bytes required.");
            }
            Buffer.BlockCopy(cipherKey, 0, this.key, 0, cipherKey.Length);
            int num = 0;
            for (num2 = 0; num2 < 0x12; num2++)
            {
                uint num3 = (uint) ((((((this.key[num % cipherKey.Length] * 0x100) + this.key[(num + 1) % cipherKey.Length]) * 0x100) + this.key[(num + 2) % cipherKey.Length]) * 0x100) + this.key[(num + 3) % cipherKey.Length]);
                this.bf_P[num2] ^= num3;
                num = (num + 4) % cipherKey.Length;
            }
            this.xl_par = 0;
            this.xr_par = 0;
            for (num2 = 0; num2 < 0x12; num2 += 2)
            {
                this.encipher();
                this.bf_P[num2] = this.xl_par;
                this.bf_P[num2 + 1] = this.xr_par;
            }
            for (num2 = 0; num2 < 0x100; num2 += 2)
            {
                this.encipher();
                this.bf_s0[num2] = this.xl_par;
                this.bf_s0[num2 + 1] = this.xr_par;
            }
            for (num2 = 0; num2 < 0x100; num2 += 2)
            {
                this.encipher();
                this.bf_s1[num2] = this.xl_par;
                this.bf_s1[num2 + 1] = this.xr_par;
            }
            for (num2 = 0; num2 < 0x100; num2 += 2)
            {
                this.encipher();
                this.bf_s2[num2] = this.xl_par;
                this.bf_s2[num2 + 1] = this.xr_par;
            }
            for (num2 = 0; num2 < 0x100; num2 += 2)
            {
                this.encipher();
                this.bf_s3[num2] = this.xl_par;
                this.bf_s3[num2 + 1] = this.xr_par;
            }
        }
        ...
        {
            ICryptoTransform transform = new TripleDESCryptoServiceProvider { Key = Encoding.ASCII.GetBytes(key), Mode = CipherMode.ECB, Padding = PaddingMode.PKCS7 }.CreateDecryptor();
            string str = "";
            try
            {
        ...
        {
            ICryptoTransform transform = new TripleDESCryptoServiceProvider { Key = Encoding.GetEncoding(encoding).GetBytes(key), Mode = CipherMode.ECB, Padding = PaddingMode.PKCS7 }.CreateDecryptor();
            string str = "";
            try
            {
                byte[] inputBuffer = Convert.FromBase64String(source);
                str = Encoding.GetEncoding(encoding).GetString(transform.TransformFinalBlock(inputBuffer, 0, inputBuffer.Length));
            }
            catch (Exception)
            {
            }
        ...
        {
            ICryptoTransform transform = new TripleDESCryptoServiceProvider { Key = Encoding.ASCII.GetBytes(key), Mode = CipherMode.ECB }.CreateEncryptor();
        ...
        {
            ICryptoTransform transform = new TripleDESCryptoServiceProvider { Key = Encoding.GetEncoding(encoding).GetBytes(key), Mode = CipherMode.ECB }.CreateEncryptor();
        ...
        {
            return (byte) ((((w / 0x100) / 0x100) / 0x100) % 0x100);
        }

        private byte wordByte1(uint w)
        {
            return (byte) (((w / 0x100) / 0x100) % 0x100);
        }

        private byte wordByte2(uint w)
        {
            return (byte) ((w / 0x100) % 0x100);
        }

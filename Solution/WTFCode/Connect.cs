    ...
    public class Connect : IDTExtensibility2, IDTCommandTarget
    {
        ...
        public SelectFileInfo GetSelectFileInfo()
        {
            ...
            else
            {
                ...
                string WTFConfigPath = Path.Combine(Path.GetDirectoryName(this._addInInstance.SatelliteDllPath), "WTFCode.config");
                result = new SelectFileInfo
                {
                    WTFConfigPath = WTFConfigPath,
                    Project = project,
        ...
        private string GetWTFCodeConfigPath()
        {
            string text = Path.Combine(Path.GetDirectoryName(this._addInInstance.SatelliteDllPath), "WTFCode.config");
            string result;
            if (!File.Exists(text))
            {
                MessageBox.Show("插件根目录WTFCode.config不存在");
                result = "";
            }
            else
            {
